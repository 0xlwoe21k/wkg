...
func TestDomainMoniter_UpdateDomainInfo(t *testing.T) {
	...
	if err != nil {
		fmt.Println("[!] domainService.go sql domain cache failed. line:83,   [", err, "]")
		return
	}
	...
	if err != nil {
		fmt.Println("[!] domainService.go save domain cache failed. line:75.  [", err, "]")
		return
	}
}
...
func TestDetDomaininfo(t *testing.T) {
	...
	res := subdomainscan2.DomainBrute("missfresh.cn")
	for _,v:=range res {
		fmt.Println(v)
	}
	//var s = "123"
