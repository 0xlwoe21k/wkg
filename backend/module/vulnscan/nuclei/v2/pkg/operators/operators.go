...
type Operators struct {
	...
	Matchers []*matchers2.Matcher `yaml:"matchers,omitempty" jsonschema:"title=matchers to run on response,description=Detection mechanism to identify whether the request was successful by doing pattern matching"`
	// description: |
	//   Extractors contains the extraction mechanism for the request to identify
	//   and extract parts of the response.
	Extractors []*extractors2.Extractor `yaml:"extractors,omitempty" jsonschema:"title=extractors to run on response,description=Extractors contains the extraction mechanism for the request to identify and extract parts of the response"`
	// description: |
	...
	matchersCondition matchers2.ConditionType
}
...
func (r *Operators) Compile() error {
	if r.MatchersCondition != "" {
		r.matchersCondition = matchers2.ConditionTypes[r.MatchersCondition]
	} else {
		r.matchersCondition = matchers2.ORCondition
	}
...
func (r *Operators) GetMatchersCondition() matchers2.ConditionType {
...
type MatchFunc func(data map[string]interface{}, matcher *matchers2.Matcher) bool

// ExtractFunc performs extracting operation for an extractor on model and returns true or false.
type ExtractFunc func(data map[string]interface{}, matcher *extractors2.Extractor) map[string]struct{}
...
func (r *Operators) Execute(data map[string]interface{}, match MatchFunc, extract ExtractFunc) (*Result, bool) {
	...
	for _, matcher := range r.Matchers {
		...
		if !match(data, matcher) {
			...
			if matcherCondition == matchers2.ANDCondition {
			...
			if matcherCondition == matchers2.ORCondition && matcher.Name != "" {
