...
func compileWorkflow(path string, preprocessor Preprocessor, options *protocols2.ExecuterOptions, workflow *workflows2.Workflow, loader model2.WorkflowLoader) {
...
func parseWorkflow(preprocessor Preprocessor, workflow *workflows2.WorkflowTemplate, options *protocols2.ExecuterOptions, loader model2.WorkflowLoader) error {
...
func parseWorkflowTemplate(workflow *workflows2.WorkflowTemplate, preprocessor Preprocessor, options *protocols2.ExecuterOptions, loader model2.WorkflowLoader, noValidate bool) error {
	var paths []string
	...
	for _, path := range paths {
		opts := protocols2.ExecuterOptions{
		...
		workflow.Executers = append(workflow.Executers, &workflows2.ProtocolExecuterPair{
