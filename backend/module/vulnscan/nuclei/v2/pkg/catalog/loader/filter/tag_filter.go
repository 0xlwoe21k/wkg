...
type TagFilter struct {
	...
	severities  map[severity2.Severity]struct{}
...
func (tagFilter *TagFilter) Match(templateTags, templateAuthors []string, templateSeverity severity2.Severity, extraTags []string) (bool, error) {
...
func isSeverityMatch(tagFilter *TagFilter, templateSeverity severity2.Severity) bool {
	if len(tagFilter.severities) == 0 || templateSeverity == severity2.Undefined {
		return true
	}
...
func New(config *Config) *TagFilter {
	filter := &TagFilter{
		...
		severities:  make(map[severity2.Severity]struct{}),
