...
import (
	...
	format2 "backend/module/vulnscan/nuclei/v2/pkg/reporting/format"
...
func (i *Integration) CreateNewIssue(event *output2.ResultEvent) error {
	summary := format2.Summary(event)
...
func (i *Integration) CreateIssue(event *output2.ResultEvent) error {
...
func (i *Integration) FindExistingIssue(event *output2.ResultEvent) (string, error) {
	template := format2.GetMatchedTemplate(event)
...
func jiraFormatDescription(event *output2.ResultEvent) string { // TODO remove the code duplication: format.go <-> jira.go
	template := format2.GetMatchedTemplate(event)
	...
	builder.WriteString(format2.ToMarkdownTableString(&event.Info))
	...
	if len(event.ExtractedResults) > 0 || len(event.Metadata) > 0 {
		...
		if len(event.Metadata) > 0 {
			...
			for k, v := range event.Metadata {
				...
				builder.WriteString(": ")
				builder.WriteString(types2.ToString(v))
				builder.WriteString("\n")
			}
			builder.WriteString("\n")
		}
	...
	builder.WriteString(fmt.Sprintf("\n---\nGenerated by [Nuclei v%s](https://github.com/projectdiscovery/nuclei)", config2.Version))
	data := builder.String()
	return data
}
