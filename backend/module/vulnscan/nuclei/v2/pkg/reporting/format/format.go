...
func Summary(event *output2.ResultEvent) string {
	template := GetMatchedTemplate(event)

	builder := &strings.Builder{}
	builder.WriteString(types2.ToString(event.Info.Name))
	builder.WriteString(" (")
...
func MarkdownDescription(event *output2.ResultEvent) string { // TODO remove the code duplication: format.go <-> jira.go
	template := GetMatchedTemplate(event)
	...
	builder.WriteString(fmt.Sprintf("\n---\nGenerated by [Nuclei %s](https://github.com/projectdiscovery/nuclei)", config2.Version))
	data := builder.String()
	return data
}

// GetMatchedTemplate returns the matched template from a result event
func GetMatchedTemplate(event *output2.ResultEvent) string {
	builder := &strings.Builder{}
...
func ToMarkdownTableString(templateInfo *model2.Info) string {
	fields := utils2.NewEmptyInsertionOrderedStringMap(5)
	...
	toMarkDownTable := func(insertionOrderedStringMap *utils2.InsertionOrderedStringMap) {
		insertionOrderedStringMap.ForEach(func(key string, value string) {
			if utils2.IsNotBlank(value) {
	...
	toMarkDownTable(utils2.NewInsertionOrderedStringMap(templateInfo.Metadata))

	return builder.String()
}

func generateCVSSMetricsFromClassification(classification *model2.Classification, fields *utils2.InsertionOrderedStringMap) {
	// Generate cvss link
...
func generateCVECWEIDLinksFromClassification(classification *model2.Classification, fields *utils2.InsertionOrderedStringMap) {
