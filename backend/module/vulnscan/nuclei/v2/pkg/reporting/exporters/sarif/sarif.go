...
func (i *Exporter) Export(event *output2.ResultEvent) error {
	...
	fullDescription := format2.MarkdownDescription(event)
	sarifSeverity := getSarifSeverity(event)

	var ruleName string
	if utils2.IsNotBlank(event.Info.Name) {
	...
	if utils2.IsNotBlank(event.Info.Description) {
...
func getSarifSeverity(event *output2.ResultEvent) string {
	switch event.Info.SeverityHolder.Severity {
	case severity2.Info:
		return "note"
	case severity2.Low, severity2.Medium:
		return "warning"
	case severity2.High, severity2.Critical:
		return "error"
	default:
		return "note"
	}
