...
func TestNetworkExecuteWithResults(t *testing.T) {
	...
	t.Run("domain-valid", func(t *testing.T) {
		...
		err := request.ExecuteWithResults(parsed.Host, metadata, previous, func(event *output2.InternalWrappedEvent) {
			finalEvent = event
		})
	...
	t.Run("invalid-port-override", func(t *testing.T) {
		metadata := make(output2.InternalEvent)
		previous := make(output2.InternalEvent)
		err := request.ExecuteWithResults("127.0.0.1:11211", metadata, previous, func(event *output2.InternalWrappedEvent) {
			finalEvent = event
		})
		require.Nil(t, err, "could not execute network request")
	})
