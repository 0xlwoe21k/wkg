...
func (r *Request) ExecuteWithResults(input string, metadata /*TODO review unused parameter*/, previous output2.InternalEvent, callback protocols2.OutputEventCallback) error {
	...
	for _, kv := range r.addresses {
		actualAddress := replacer2.Replace(kv.ip, map[string]interface{}{"Hostname": address})
		if kv.port != "" {
...
func (r *Request) executeAddress(actualAddress, address, input string, shouldUseTLS bool, previous output2.InternalEvent, callback protocols2.OutputEventCallback) error {
...
func (r *Request) executeRequestWithPayloads(actualAddress, address, input string, shouldUseTLS bool, payloads map[string]interface{}, previous output2.InternalEvent, callback protocols2.OutputEventCallback) error {
	var (
		hostname string
		conn     net.Conn
		err      error
	...
	for _, input := range r.Inputs {
		...
		finalData, dataErr := expressions2.EvaluateByte(data, payloads)
		if dataErr != nil {
