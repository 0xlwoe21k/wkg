...
func (r *Request) responseToDSLMap(req, resp, raw, host, matched string) output2.InternalEvent {
	data := make(output2.InternalEvent, 6)
...
func (r *Request) makeResultEventItem(wrapped *output2.InternalWrappedEvent) *output2.ResultEvent {
	data := &output2.ResultEvent{
		...
		Response:         types2.ToString(wrapped.InternalEvent["data"]),
	}
	return data
}
