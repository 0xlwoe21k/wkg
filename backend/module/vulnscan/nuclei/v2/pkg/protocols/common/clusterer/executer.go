...
type clusteredOperator struct {
	...
	operator     *operators2.Operators
}

var _ protocols2.Executer = &Executer{}

// NewExecuter creates a new request executer for list of requests
func NewExecuter(requests []*templates2.Template, options *protocols2.ExecuterOptions) *Executer {
	executer := &Executer{
		options:  options,
...
func (e *Executer) Execute(input string) (bool, error) {
	...
	err := e.requests.ExecuteWithResults(input, dynamicValues, previous, func(event *output2.InternalWrappedEvent) {
...
func (e *Executer) ExecuteWithResults(input string, callback protocols2.OutputEventCallback) error {
	dynamicValues := make(map[string]interface{})
	err := e.requests.ExecuteWithResults(input, dynamicValues, nil, func(event *output2.InternalWrappedEvent) {
		for _, operator := range e.operators {
			result, matched := operator.operator.Execute(event.InternalEvent, e.requests.Match, e.requests.Extract)
			if matched && result != nil {
				event.OperatorsResult = result
