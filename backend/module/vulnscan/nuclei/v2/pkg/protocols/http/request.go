...
import (
	...
	httpclientpool2 "backend/module/vulnscan/nuclei/v2/pkg/protocols/http/httpclientpool"
	"bytes"
	"fmt"
	"io"
	"io/ioutil"
...
func (r *Request) executeRaceRequest(reqURL string, previous output2.InternalEvent, callback protocols2.OutputEventCallback) error {
...
func (r *Request) executeParallelHTTP(reqURL string, dynamicValues output2.InternalEvent, callback protocols2.OutputEventCallback) error {
	generator := r.newGenerator()
...
func (r *Request) executeTurboHTTP(reqURL string, dynamicValues, previous output2.InternalEvent, callback protocols2.OutputEventCallback) error {
	generator := r.newGenerator()
...
func (r *Request) ExecuteWithResults(reqURL string, dynamicValues, previous output2.InternalEvent, callback protocols2.OutputEventCallback) error {
	...
	var requestErr error
	for {
		hasInteractMarkers := interactsh2.HasMatchers(r.CompiledOperators)
		...
		var gotOutput bool
		r.options.RateLimiter.Take()
		err = r.executeRequest(reqURL, request, previous, hasInteractMarkers, func(event *output2.InternalWrappedEvent) {
			// Add the extracts to the dynamic values if any.
			if event.OperatorsResult != nil {
				gotOutput = true
				dynamicValues = generators2.MergeMaps(dynamicValues, event.OperatorsResult.DynamicValues)
			}
			if hasInteractMarkers && r.options.Interactsh != nil {
				r.options.Interactsh.RequestEvent(interactURL, &interactsh2.RequestData{
...
func (r *Request) executeRequest(reqURL string, request *generatedRequest, previous output2.InternalEvent, hasInteractMarkers bool, callback protocols2.OutputEventCallback, requestCount int) error {
	...
	if request.original.Pipeline {
		if request.rawRequest != nil {
			...
			resp, err = request.pipelinedClient.DoRaw(request.rawRequest.Method, reqURL, request.rawRequest.Path, generators2.ExpandMapValues(request.rawRequest.Headers), ioutil.NopCloser(strings.NewReader(request.rawRequest.Data)))
		...
		resp, err = request.original.rawhttpClient.DoRawWithOptions(request.rawRequest.Method, reqURL, request.rawRequest.Path, generators2.ExpandMapValues(request.rawRequest.Headers), ioutil.NopCloser(strings.NewReader(request.rawRequest.Data)), options)
	} else {
	...
	if err != nil {
		...
		if hasInteractMarkers {
			outputEvent := r.responseToDSLMap(&http.Response{}, reqURL, formedURL, tostring2.UnsafeToString(dumpedRequest), "", "", "", 0, request.meta)
			if i := strings.LastIndex(hostname, ":"); i != -1 {
				hostname = hostname[:i]
			}
			outputEvent["ip"] = httpclientpool2.Dialer.GetDialedIP(hostname)
	...
	finalEvent := make(output2.InternalEvent)

	outputEvent := r.responseToDSLMap(resp, reqURL, matchedURL, tostring2.UnsafeToString(dumpedRequest), tostring2.UnsafeToString(dumpedResponse), tostring2.UnsafeToString(data), headersToString(resp.Header), duration, request.meta)
	if i := strings.LastIndex(hostname, ":"); i != -1 {
		hostname = hostname[:i]
	}
	outputEvent["ip"] = httpclientpool2.Dialer.GetDialedIP(hostname)
	outputEvent["redirect-chain"] = tostring2.UnsafeToString(redirectedResponse)
