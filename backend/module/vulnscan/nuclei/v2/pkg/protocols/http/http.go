...
type Request struct {
	...
	operators2.Operators `yaml:",inline"`
	// description: |
	...
	attackType    generators2.Type
	totalRequests int
	customHeaders map[string]string
	generator     *generators2.Generator // optional, only enabled when using payloads
...
func (r *Request) Compile(options *protocols2.ExecuterOptions) error {
	connectionConfiguration := &httpclientpool2.Configuration{
		Threads:         r.Threads,
	...
	if _, hasConnectionHeader := r.Headers["Connection"]; hasConnectionHeader {
		connectionConfiguration.Connection = &httpclientpool2.ConnectionConfiguration{DisableKeepAlive: false}
	}

	client, err := httpclientpool2.Get(options.Options, connectionConfiguration)
	if err != nil {
	...
	if len(r.Raw) > 0 {
		...
		r.rawhttpClient = httpclientpool2.GetRawHTTP(options.Options)
	}
	...
	if len(r.Payloads) > 0 {
		...
		r.attackType = generators2.StringToType[attackType]
		...
		r.generator, err = generators2.New(r.Payloads, r.attackType, r.options.TemplatePath)
		if err != nil {
