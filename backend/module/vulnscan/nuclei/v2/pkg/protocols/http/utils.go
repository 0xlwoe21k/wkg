...
func dumpResponseWithRedirectChain(resp *http.Response, body []byte) ([]byte, error) {
	...
	redirectChain.WriteString(tostring2.UnsafeToString(respData))
	...
	for redirectResp != nil {
		...
		redirectChain.WriteString(tostring2.UnsafeToString(respData))
		if len(body) > 0 {
			redirectChain.WriteString(tostring2.UnsafeToString(body))
		}
		redirects = append(redirects, redirectChain.String())
...
func dump(req *generatedRequest, reqURL string) ([]byte, error) {
	...
	return rawhttp.DumpRequestRaw(req.rawRequest.Method, reqURL, req.rawRequest.Path, generators2.ExpandMapValues(req.rawRequest.Headers), ioutil.NopCloser(strings.NewReader(req.rawRequest.Data)), rawhttp.Options{CustomHeaders: req.rawRequest.UnsafeHeaders, CustomRawBytes: req.rawRequest.UnsafeRawBytes})
}
