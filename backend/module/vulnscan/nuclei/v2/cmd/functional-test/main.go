...
func runIndividualTestCase(testcase string) error {
	...
	mainOutput, err := testutils2.RunNucleiBinaryAndGetLoadedTemplates(*mainNucleiBinary, finalArgs)
	if err != nil {
		return errors.Wrap(err, "could not run nuclei main test")
	}
	devOutput, err := testutils2.RunNucleiBinaryAndGetLoadedTemplates(*devNucleiBinary, finalArgs)
	if err != nil {
