...
import (
	runner2 "backend/module/vulnscan/nuclei/v2/internal/runner"
...
on extensive configurability, massive extensibility and ease of use.`)
	...
	*/
	...
	createGroup(flagSet, "filters", "Filtering",
		...
		flagSet.VarP(&options.Severities, "impact", "severity", fmt.Sprintf("Templates to run based on severity. Possible values: %s", severity2.GetSupportedSeverities().String())),
	...
	createGroup(flagSet, "output", "Output",
		...
		flagSet.StringVarP(&options.ReportingDB, "report-db", "rdb", "", "local nuclei reporting db (always use this to persist report data)"),
	...
	createGroup(flagSet, "configs", "Configurations",
		...
		flagSet.StringVarP(&options.ReportingConfig, "report-config", "rc", "", "nuclei reporting libs configuration file"), // TODO merge into the config file or rename to issue-tracking
