...
func (r *Runner) updateTemplates() error {
	...
	if r.templatesConfig == nil {
		currentConfig := &config2.Config{
			TemplatesDirectory: filepath.Join(home, "nuclei-templates"),
			NucleiVersion:      config2.Version,
		}
		if writeErr := config2.WriteConfiguration(currentConfig); writeErr != nil {
	...
	client.InitNucleiVersion(config2.Version)
	...
	if r.templatesConfig.TemplateVersion == "" || (r.options.TemplatesDirectory != "" && r.templatesConfig.TemplatesDirectory != r.options.TemplatesDirectory) || noTemplatesFound {
		...
		r.templatesConfig = &config2.Config{
			TemplatesDirectory: filepath.Join(home, "nuclei-templates"),
		}
		...
		if err := config2.WriteConfiguration(r.templatesConfig); err != nil {
			return err
		}
	...
	if version.EQ(oldVersion) {
		...
		return config2.WriteConfiguration(r.templatesConfig)
	}
...
func (r *Runner) readInternalConfigurationFile(home, configDir string) error {
	...
	if _, statErr := os.Stat(templatesConfigFile); !os.IsNotExist(statErr) {
		configuration, readErr := config2.ReadConfiguration()
		if readErr != nil {
			return readErr
		}
