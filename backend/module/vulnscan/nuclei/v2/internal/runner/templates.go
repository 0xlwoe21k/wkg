...
func (r *Runner) templateLogMsg(id, name string, authors []string, templateSeverity severity2.Severity) string {
...
func (r *Runner) logAvailableTemplate(tplPath string) {
	t, err := parsers2.ParseTemplate(tplPath)
	if err != nil {
		gologger.Error().Msgf("Could not parse file '%s': %s\n", tplPath, err)
	} else {
		gologger.Print().Msgf("%s\n", r.templateLogMsg(t.ID,
			types2.ToString(t.Info.Name),
